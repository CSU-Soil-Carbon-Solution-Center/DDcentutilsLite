...
#'
#' @description  This function retrieves the Daymet weather data of a specific site based on its latitude and longitude
#' using the `daymetr` package. The output includes weather variables necessary for a DayCent run: day of the month,
...
#'
#' @details
#' The function downloads Daymet weather data based on a user-specified time interval,
#' and adds an additional row for day 366 in leap years.
#'
#' @examples
#' # Example usage
#' lat <- 40.58
#' lon <- -105.08
#' site <- "Sample Site"
#' weather <- getWeatherData(raw_data_path = NULL, site = site,
#'                           lat = lat, lon = lon, start = 2000, end = 2010)
#'
#' @import daymetr
#' @import dplyr
#' @export
getWeatherData <- function(raw_data_path = NULL, site = site,
                           lat = lat, lon = lon,
                           start = start, end = end, ...){
  #load data
  #description of the data https://daymet.ornl.gov/overview
  ...
  weather_data_DayCent <- weather_data %>% mutate(
    ...
    VPD_kpa_day = NULL # calVPDfromVP(TAvg_c =(tmax_C+tmin_C)/2, VP_kpA = `vp (Pa)`/1000)
  ) %>%
